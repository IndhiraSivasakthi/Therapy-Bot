        ...
        <div className="max-w-2xl mx-auto flex flex-col h-[90vh] border shadow-md rounded-lg overflow-hidden"
          ...
          <div className="p-4 flex justify-between items-center border-b"
            ...
            <div className="space-x-2">
              <button onClick={toggleTheme} className="px-3 py-1 rounded text-sm"
                      style={{ backgroundColor: darkMode ? "#fbbf24" : "#1f2937", color: darkMode ? "#000" : "#fff" }}>
                {darkMode ? '☀️ Light' : '🌙 Dark'}
              </button>
              <button onClick={handleExport} className="px-3 py-1 bg-green-600 text-white rounded text-sm">
                Export Chat
              </button>
              <Link href="/login" className="px-3 py-1 text-sm text-blue-400 hover:underline">
                Logout
              </Link>
            </div>
          </div>
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className="px-4 py-2 rounded-lg max-w-xs break-words text-sm"
                     style={{ backgroundColor: msg.sender === 'user' ? "#3b82f6" : darkMode ? "#374151" : "#e5e7eb", color: msg.sender === 'user' ? "#fff" : darkMode ? "#fff" : "#000" }}>
          ...
          <div className="p-4 flex flex-col space-y-2 border-t" style={{ borderColor: darkMode ? "#374151" : "#e5e7eb" }}>
            <select onChange={e => setInput(e.target.value)} value="" className="p-2 rounded border text-sm"
                    style={{ backgroundColor: darkMode ? "#111827" : "#fff", color: darkMode ? "#fff" : "#000", borderColor: darkMode ? "#4b5563" : "#d1d5db" }}>
            ...
            <div className="flex">
              <input type="text" className="flex-1 p-2 rounded-l-lg border outline-none"
                     style={{ backgroundColor: darkMode ? "#111827" : "#fff", color: darkMode ? "#fff" : "#000", borderColor: darkMode ? "#4b5563" : "#d1d5db" }}
                     placeholder="How are you feeling?" value={input} onChange={e => setInput(e.target.value)}
                     onKeyDown={e => e.key === "Enter" && handleSend()} />
              <button onClick={handleSend} className="bg-blue-600 text-white px-4 rounded-r-lg hover:bg-blue-700">
                Send
              </button>
            </div>
          </div>
